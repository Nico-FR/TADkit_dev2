...
matObsExp_v2 <- function(matrix, matrix.colname = T, matrix.rowname = T, matrix.sep = "\t") {
  ...
  #replace gap (i.e bin without reads) with NA values instead of 0
  sum_line = sapply(1:ncol(mat), function (x){sum(mat[x,], na.rm=TRUE)})
  mat[sum_line == 0, ] <- NA
