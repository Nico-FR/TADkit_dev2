...
Now you should be able to install the development version of TADkit from [GitHub](https://github.com/) with:
...
In addition to TADs, we can also plot any type of annotations. For this tutorial we will use Human annotations available as a package (TxDb.Btaurus.UCSC.bosTau9.refGene):
```{r}
...
write.table(IS_HCT116_chr19_5kb.bedgraph, "./IS_HCT116_chr19_5kb_1.bedgraph", col.names = FALSE, quote = FALSE, row.names = FALSE, sep = "\t") 
...
write.table(
  ...
  "./IS_HCT116_chr19_5kb_2.bedgraph", col.names = FALSE, quote = FALSE, row.names = FALSE, sep = "\t") 
```
...
IS.lst = list(ind1 = "./IS_HCT116_chr19_5kb_1.bedgraph", ind2 = "./IS_HCT116_chr19_5kb_2.bedgraph")
```
...
Many algorithms allow to compute from an HiC matrix values of a principal component which allows to identify compartments A or B. The orientation of these values is random and generally it is secondly oriented using gene density or GC percent between compartments A and B. 
...
To do so, we will use the package "airway" which gives read counts in genes for an RNA-Seq experiment human airway smooth muscle cell lines. Although the cell line has little to do with the HiC data used, the 3D organization is sufficiently consistent between cell types to verify the orientation of compartments.
