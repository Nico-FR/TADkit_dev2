...
Two distinct approaches allow to analyzed and visualized distributions of any annotation features according to the domains: 

Thus, the distribution of features can be analyzed:

* on both sides of TAD boundaries (around boudaries),
* or within TADs.
...
```{r, fig.height = 5, fig.width = 5}
MATfeatures(matrix = matrix_1_chr25_50kb, bin.width = 50e3, annot.gr = genes.gr, chr = 25, annot.boundary = "start", window.size = 1e6, output = "plot")
```
...
data = compOrientation(bedgraph.gr = PC1_1_50kb.gr, annot.gr = genes.gr, expression.data.frame = expression.data.frame)
```
